...
public class COSUtil {
  ...
  public TransferManager createTransferManager() {
    ...
    // 自定义线程池大小，建议在客户端与 COS 网络充足（例如使用腾讯云的
    // CVM，同地域上传 COS）的情况下，设置成16或32即可，可较充分的利用网络资源
    // 对于使用公网传输且网络带宽质量不高的情况，建议减小该值，避免因网速过慢，造成请求超时。
    ExecutorService threadPool = Executors.newFixedThreadPool(16);

    // 传入一个 threadpool, 若不传入线程池，默认 TransferManager
    // 中会生成一个单线程的线程池。
  ...
  public String upload(String keyName, MultipartFile multipartFile) {
    ...
    // file里面填写本地图片的位置
    // 我这里是相对项目的位置，在项目下有src/test/demo.jpg这张图片
    ...
    PutObjectRequest putObjectRequest = new PutObjectRequest(
        cosClientConfig.getBucket(), keyName, inputStream, objectMetadata);

    TransferManager transferManager = this.createTransferManager();

    try {
      // 高级接口会返回一个异步结果Upload
      // 可同步地调用 waitForUploadResult 方法等待上传完成，成功返回
      // UploadResult, 失败抛出异常
  ...
  public void deleteVideoFile(String md5, String type) {
    DeleteObjectsRequest deleteObjectsRequest =
        new DeleteObjectsRequest(cosClientConfig.getBucket());
    // 设置要删除的key列表, 最多一次删除1000个
    ArrayList<DeleteObjectsRequest.KeyVersion> keyList = new ArrayList<>();

    keyList.add(
        new DeleteObjectsRequest.KeyVersion("bilibili/" + md5 + "." + type));
    keyList.add(new DeleteObjectsRequest.KeyVersion("bilibili/流畅" + md5 +
                                                    "." + type));
    keyList.add(new DeleteObjectsRequest.KeyVersion("bilibili/高清" + md5 +
                                                    "." + type));
    keyList.add(new DeleteObjectsRequest.KeyVersion("bilibili/标清" + md5 +
                                                    "." + type));
    keyList.add(new DeleteObjectsRequest.KeyVersion("bilibili/超清" + md5 +
                                                    "." + type));
  ...
  public void deleteAdvertisementFile(String md5, String type) {
    cosClient.deleteObject(cosClientConfig.getBucket(),
                           "bilibili/advertisement" + md5 + "." + type);
  }

  public String initiateMultipartUpload(String key) {
    InitiateMultipartUploadRequest request =
        new InitiateMultipartUploadRequest(cosClientConfig.getBucket(), key);
  ...
  public PartETag uploadPart(MultipartFile file, String uploadId,
                             Integer slicerNumber, String key) {
  ...
  public URL completeMultipartUpload(String key, String uploadIdKey,
                                     String uploadedNumberKey) {
    ObjectMapper objectMapper = new ObjectMapper();
    // 保存已上传的分片信息，实际情况里，这里的内容是从上传分块接口中获取到的
    List<PartETag> partETags = new LinkedList<>();

    int number = Integer.parseInt(Objects.requireNonNull(
        redisTemplate.opsForValue().get(uploadedNumberKey)));
    for (int i = 1; i <= number; i++) {
      try {
        PartETag partETag = objectMapper.readValue(
            redisTemplate.opsForValue().get(uploadIdKey + String.valueOf(i)),
            PartETag.class);
    ...
    CompleteMultipartUploadRequest completeMultipartUploadRequest =
        new CompleteMultipartUploadRequest(cosClientConfig.getBucket(), key,
                                           uploadId, partETags);
  ...
  public String uploadFileBySlices(MultipartFile file, String fileMD5,
    ...
    if (sliceNumber == 1) { // 上传第一个分片
      ...
      redisTemplate.opsForValue().set(
          uploadIdKey + redisTemplate.opsForValue().get(uploadedNumberKey),
          objectMapper.writeValueAsString(partETag));
      ...
      PartETag partETag =
          this.uploadPart(file, uploadId, sliceNumber, key); // 上传分片
    ...
    redisTemplate.opsForValue().set(uploadedSizeKey,
                                    String.valueOf(uploadedSize));

    // 上传完毕，清空redis值
    String uploadedNumberStr =
        redisTemplate.opsForValue().get(uploadedNumberKey);
    ...
    if (uploadedNumber != null && uploadedNumber.equals(totalSliceNumber)) {
      String uploadId = redisTemplate.opsForValue().get(uploadIdKey);
      resultPath = String.valueOf(this.completeMultipartUpload(
          uploadId, uploadIdKey, uploadedNumberKey)); // 完成分片上传
      ...
      List<String> keyList =
          Arrays.asList(uploadedNumberKey, uploadIdKey, uploadedSizeKey);
