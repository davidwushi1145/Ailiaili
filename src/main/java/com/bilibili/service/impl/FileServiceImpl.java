...
public class FileServiceImpl
    extends ServiceImpl<FileMapper, File> implements FileService {
  ...
  public String uploadFileBySlices(MultipartFile slice, String fileMD5,
    ...
    if (totalSliceNumber == 1) {
      // COS对象存储
      filePath = cosUtil.upload(
          VIDEO_FILE_NAME + fileMD5 + "." + cosUtil.getFileType(slice), slice);
    } else {
      filePath = cosUtil.uploadFileBySlices(slice, fileMD5, sliceNumber,
                                            totalSliceNumber);
    }
  ...
  @Override
  public Long uploadAdvertisementFile(MultipartFile file, String fileMD5)
      throws IOException {
    if (StringUtils.isBlank(fileMD5)) {
      throw new ConditionException("参数异常！");
    }
    ...
    String filePath;
    // COS对象存储
    filePath = cosUtil.upload(ADVERTISEMENT_FILE_NAME + fileMD5 + "." +
                                  cosUtil.getFileType(file),
                              file);
  ...
  @Override
  public String uploadThumbnailFile(MultipartFile file, String fileMD5)
      throws IOException {
    if (StringUtils.isBlank(fileMD5)) {
      throw new ConditionException("参数异常！");
    }
    ...
    // COS对象存储
    filePath = cosUtil.upload(
        THUMBNAIL_FILE_NAME + fileMD5 + "." + cosUtil.getFileType(file), file);
