...
public class FileServiceImpl extends ServiceImpl<FileMapper, File>
    ...
    public String uploadFileBySlices(MultipartFile slice, String fileMD5, Integer sliceNumber, Integer totalSliceNumber) throws IOException {
        File dbFileMD5 = this.getFileByMD5(fileName + slice.getOriginalFilename());
        //秒传
        if (dbFileMD5 != null) {
        ...
        //COS对象存储
        String filePath = cosUtil.upload(fileName + slice.getOriginalFilename(), slice);

        if (StringUtils.isNotBlank(filePath)) {
            dbFileMD5 = new File();
            dbFileMD5.setUrl(filePath);
            dbFileMD5.setMd5(fileName + slice.getOriginalFilename());
