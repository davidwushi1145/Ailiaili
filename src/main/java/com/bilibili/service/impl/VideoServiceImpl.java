...
import com.bilibili.dao.mapper.VideoMapper;
...
public class VideoServiceImpl
    extends ServiceImpl<VideoMapper, Video> implements VideoService {

  @Autowired private VideoTagService videoTagService;

  @Autowired private FastDFSUtil fastDFSUtil;
  ...
  @Override
  public PageResult<Video> pageListVideos(Integer size, Integer page,
                                          String area) {
  ...
  public List<Video> recommend(Long userId) throws TasteException {
    List<UserPreference> userPreferenceList =
        videoOperationService.getAllUserPreference();
    // 创建数据模型
    ...
    Recommender recommender = new GenericUserBasedRecommender(
        dataModel, userNeighborhood, similarity);
    // 推荐商品
    List<RecommendedItem> recommendedItems = recommender.recommend(userId, 5);
    List<Long> collect = recommendedItems.stream()
                             .map(RecommendedItem::getItemID)
                             .collect(Collectors.toList());
    // 检查推荐列表是否为空
    if (collect.isEmpty()) {
      // 返回一个者默认推荐
      return Collections.emptyList(); // 返回默认推荐视频列表
    }
  ...
  private DataModel createDataModel(List<UserPreference> userPreferenceList) {
    ...
    Map<Long, List<UserPreference>> map = userPreferenceList.stream().collect(
        Collectors.groupingBy(UserPreference::getUserId));
    ...
    for (List<UserPreference> userPreferences : list) {
      ...
      for (int i = 0; i < userPreferences.size(); i++) {
        ...
        GenericPreference item = new GenericPreference(
            userPreference.getUserId(), userPreference.getVideoId(),
            userPreference.getValue());
        array[i] = item;
      }
      fastByIdMap.put(array[0].getUserID(),
                      new GenericUserPreferenceArray(Arrays.asList(array)));
    }
