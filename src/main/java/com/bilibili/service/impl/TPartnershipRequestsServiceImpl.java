 ...
 * @author 下水道的小老鼠
 * @description
 * 针对表【t_partnership_requests(合作请求表)】的数据库操作Service实现
 * @createDate 2023-12-11 11:06:25
 */
@Service
public class TPartnershipRequestsServiceImpl
    extends ServiceImpl<TPartnershipRequestsMapper, TPartnershipRequests>
    implements TPartnershipRequestsService {

  @Autowired private TPartnershipRequestsMapper tPartnershipRequestsMapper;

  @Override
  public void createPartnershipRequest(Long advertiserId, Long contentCreatorId,
  ...
  public void acceptPartnershipRequest(Long requestId) {
    ...
    TPartnershipRequests tPartnershipRequests =
        tPartnershipRequestsMapper.selectById(requestId);
    tPartnershipRequests.setStatus("accepted");
    tPartnershipRequestsMapper.update(tPartnershipRequests, queryWrapper);
  }

  @Override
  public List<TPartnershipRequests> getTPartnershipRequests(Long userId,
                                                            String status) {
    QueryWrapper<TPartnershipRequests> queryWrapper = new QueryWrapper<>();
    ...
    if (StringUtils.isNotEmpty(status)) {
      ...
      queryWrapper.and(
          wrapper
          -> wrapper.eq("status", "accepted").or().eq("status", "rejected"));
    }
